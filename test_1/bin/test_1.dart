...
class Room {
  ...
  void AddPlayer(User? user) {
    if (user == null) return;
    if (userList.isEmpty) {
      user.master = true; // 첫 접속 시 마스터 권한 부여.
    }
    userList.add(user);
  }
...
class Game {
  ...
  Vector2 getBallPos(double globalTime) {
    ...
    double changedY =
        changedX * (vs / hs) - 0.5 * gravity * pow(changedX / hs, 2);
    return Vector2(ball.startPos.x + changedX, ball.startPos.y + changedY);
  }
}

Future<User?> upgradeToSocket(HttpRequest req) async {
  // Upgrade an HttpRequest to a WebSocket connection
  try {
    WebSocket socket = await WebSocketTransformer.upgrade(req);
    User user = User(socket);
    print('Client connected!');
    user.socket.add('성공적으로 방에 연결 됐습니다.');
    // Listen for incoming messages from the client
    user.socket.listen((message) {
      print("유저로부터 메시지 ${message}");
      if (message == 'ready') {
        user.isReady = true;
        print("유저 준비");
      }
...
void JoinRoom(int roomIdx, User user) {
  if (roomIdx >= 0 && roomIdx < roomList.length + 1) {
    final room = roomList[roomIdx];
    room.AddPlayer(user);
  }
}
...
void handleJoinRoomRequest(HttpRequest req) async {
  ...
  if (roomIndex >= 0 && roomIndex < roomList.length + 1) {
    // 방 인덱스 유효 시
    print("서버에 연결 시도...");
    User? user = await upgradeToSocket(req);

    if (user != null) {
      JoinRoom(roomIndex, user); // 방 접속
    } else {
      print('방 접속 실패...');
      req.response
        ..statusCode = HttpStatus.ok
